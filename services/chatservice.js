...
async function deleteFlowiseSession(roomId) {
  try {
    const url = `http://101.50.2.61:3000/api/v1/chatmessage/96e09b89-9cc6-44d9-a3ca-50978d0a1fe1?sessionId=${roomId}`;
    ...
    if (resp.data && resp.data.affected) {
      ...
      console.warn(`⚠️ Unexpected response when deleting session ${roomId}`, resp.data);
    }
